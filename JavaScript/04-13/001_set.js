...
for (const i of 회사게시판) {
    ...
    // 여기 set은 자료형이 아니다 map에 값을 세팅하는 역할
}
...
let map = new Map()
map.get('이호준') // undefined 제일처음 넣을 땐 값이 없다 그래서 || 연산으로 +1
map.set('이호준', 1) // 최종
...
let a = new Set('abc')
let b = new Set('cde')
let cro = [...a].filter(e => b.has(e)) // [...a]는 배열, b는 Set자료형
// Set은 순서를 가지는 이터러블객체(순회가능한 객체)
