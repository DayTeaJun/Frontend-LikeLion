...
// 클로저는 이때 제곱(2)로 이미 선언해서 폐쇠된 공간이 되었지만, 접근할 수 있는 테크닉이다
...
function 승수제조기() {
    ...
    return 승수 // 승수()가 아닌 승수 이다.
}

let 승 = 승수제조기(); // 이미 선언된 함수의 value(폐쇠된 공간)을 수정
// 

// 은행권에서 변경가능한 자료나 그 안의 자료를 만질 수 있게 하면 난리남 그래서 은닉화함
승()
승()
승()
...
outer(); // 함수호출될 때마다 생성되고 휘발(메모리상에서 사라짐)되는게 반복 
...
var newInner = outer(); // 휘발되지않고 newInner에 저장
newInner(); // 12
